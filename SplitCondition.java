...
public class SplitCondition {
	...
	public String extractAnonCode(JSONObject jsonObject)
	{
	  ...
	  if(minIndex != -1 && maxIndex != -1)
		{
			anonCode= anonCode + searchString.substring(minIndex, maxIndex).replace("?", "").replace(" ", "");
		} else if(minIndex != -1 && maxIndex == -1){                          //added this to catch error for anon code
			anonCode= anonCode + searchString.substring(minIndex, minIndex+9).replace("?", "").replace(" ", "");
		}

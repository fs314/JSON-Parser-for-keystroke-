...
public class WriteJSON {
	...
	public void createKSFile (String originalPath, String destinationPath) throws FileNotFoundException
	{
		for (int i=0; i<filesForFolder(originalPath).size(); i++)
		{
			...
			String originalName = filesForFolder(originalPath).get(i).replace("originalKS\\", "").replace(".json", "");
			...
			printFiles(makePpsFolder(ppsNumber, destinationPath) + "s"+i+"-static", newJo);
			printFiles(makePpsFolder(ppsNumber, destinationPath) + originalName, jsonObject);
	    }
	}
